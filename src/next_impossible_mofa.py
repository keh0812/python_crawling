...
class CrawlingPagingMain():
    ...
    def crawling_main(self, collection_data):
        ...
        try:
            ...
            # 총 페이지 수 추출
            total = "#contents > .board_list > .bo_info > div"
            cnt = driver.find_element_by_css_selector(total).text
            page_total = cnt[cnt.find("/") + 1:]
            page_total = re.sub(korean, '', page_total)
            print(page_total, "===> 총 페이지 수")
            ...
            while next_exists:
                ...
                for paging_index in range (paging_cnt):
                    ...
                    paging_num = re.sub(korean, '', paging_num)
                    paging_num = paging_num.strip('\n')
                ...
                #     if next_img == "http://www.inss.re.kr/inss/images/btn/page_next.png" :
                #         time.sleep(2)
                ...
                print("int(paging_num) ===>", int(paging_num))
                paging_cnt = paging_cnt - 1
                print("paging_cnt ===>", paging_cnt)

                ## 한 탭 내 마지막 페이지 확인 (탭 이동)
                # 마지막 페이지가 아닐 때
                if paging_index != paging_cnt :

                    print("### 다음 페이지 ###")
                
                # 탭 내 마지막 페이지 일 때
                else:
                    if int(paging_num) == page_total :
                        next_exists = False
                    else :
