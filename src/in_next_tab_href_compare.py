    ...
    def crawling_main(self, collection_data):
        ...
        try:
            ...
            while next_exists:
                ...
                # 페이징 요소 찾기
                paging_all_items = driver.find_elements_by_css_selector(paging_tag)
                ...
                for item in paging_all_items:
                    ...
                    if item.text.isdecimal() == True :
                        paging_items.append(item)

                # print('paging_items : ', paging_items)
                print('페이징(숫자) 개수 : ', len(paging_items))
                paging_items_cnt = len(paging_items)
                time.sleep(2)
                ...
                for paging_index in range (paging_items_cnt):
                    ...
                    # 페이징 숫자 개수 초기화
                    paging_items_cnt = len(paging_items)
                    paging_num = paging_items[paging_index].text
                    print('페이징(숫자) 개수 : ', len(paging_items))
                    time.sleep(2)
                    ...
                    if paging_index > 0:
                        ...
                        driver.execute_script("arguments[0].scrollIntoView(true);", paging_items[paging_index])
                        if headless_height != "0":
                            driver.execute_script("window.scrollTo(0, window.scrollY - " + headless_height + ");")

                        time.sleep(2)

                        paging_items[paging_index].click()
                ...
                try:
                    ...
                    # 기상청
                    elif next_btn_href_id == "0":
                        print("### next 버튼 id = 0  - 마지막 페이지 ###")
                        next_exists = False
