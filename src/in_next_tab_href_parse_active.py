...
class CrawlingPagingMain():
    ...
    def crawling_main(self, collection_data):
        ...
        # try:
            ...
            # 페이징 정보
            paging_tag = paging_data['paging_tag']
            next_tag = paging_data['next_tag']
            active_page_tag = paging_data['active_page_tag']
                                    
            # print("paging_tag == ", paging_tag)            
            # print("next_tag == ", next_tag)
            # print("active_page_tag == ", active_page_tag)
            ...
            while next_exists:
                ...
                # 페이징 요소 찾기
                # 현재 페이지(active 페이지)와 paging_tag 요소들 합치기
                paging_items_active = driver.find_elements_by_css_selector(active_page_tag)
                paging_items = driver.find_elements_by_css_selector(paging_tag)
                paging_all_items = []
                paging_all_items = paging_items_active + paging_items

                # 숫자인 것만 담음
                paging_items_digit = []  

                for item in paging_all_items:
                    
                    # <<, < 와 숫자 구별
                    if item.text.isdecimal() == True :
                        paging_items_digit.append(item)

                # print('paging_items_digit : ', paging_items_digit)
                # print('페이징(숫자) 개수 :', len(paging_items_digit))

                # 페이징 개수
                paging_items_cnt = len(paging_items_digit)
                time.sleep(2)
               

                # 페이징 처리
                for paging_index in range (paging_items_cnt):
                    ...
                    # print("###### for ",paging_index," in range(",paging_items_cnt,") ######")

                    # 페이징 숫자 개수 초기화
                    paging_items_cnt = len(paging_items_digit)
                    paging_num = paging_items_digit[paging_index].text
                    print('페이징 개수 :', len(paging_items_digit))
                    time.sleep(2)
                    print("현재 페이지 : ", paging_num)
                    time.sleep(2)
                    
                    # 첫페이지 확인
                    if paging_index > 0:
                        
                        driver.execute_script("arguments[0].scrollIntoView(true);", paging_items_digit[paging_index])
                        if headless_height != "0":
                            driver.execute_script("window.scrollTo(0, window.scrollY - " + headless_height + ");")

                        time.sleep(2)

                        paging_items_digit[paging_index].click()
                    ...
                    # 현재 페이징 url 확인
                    current_paging_url = driver.current_url
                    print("현재 페이지 url : ", current_paging_url)
                ...
                try:
                    next_btn = driver.find_element_by_css_selector(next_tag)
                    next_btn_href = driver.find_element_by_css_selector(next_tag).get_attribute('href')
                    # print("next_btn_href : ", next_btn_href)

                    ##  url 에서 'page' 값 추출
                    url_parse = parse.urlparse(next_btn_href)
                    parse_paging_url_query= parse.parse_qs(url_parse.query)
                    # print("parse_paging_url_query : ",parse_paging_url_query)    
                
                    try :
                        
                        keys = parse_paging_url_query.keys()
                        
                        if 'page' in keys:
                            list_parse_page = list(parse_paging_url_query['page'])
                            parse_page = list(map(str, list_parse_page))
                            parse_page = parse_page[0]
                            print("href 에서 파싱한 현재 페이지 :", parse_page)
                        
                        elif 'pageIndex' in keys:
                            list_parse_page = list(parse_paging_url_query['pageIndex'])
                            parse_page = list(map(str, list_parse_page))
                            parse_page = parse_page[0]
                            print("href 에서 파싱한 현재 페이지 :", parse_page)
                    ...
                    if parse_page == paging_num :
                        print("### href 에서 파싱한 페이지와 마지막 페이지 비교 ###")
                        next_exists = False

                    # 태그 값에 none 이 포함되는 경우(마지막 페이지)
                    # https://www.humanrights.go.kr/site/program/board/basicboard/list#none
                    elif "none" in next_btn_href:
                        print("### next 버튼 클릭 불가 - 마지막 페이지 ###")
                        next_exists = False
                    ...
                    # 다음 페이지 있는 경우
                        ...
                        ### 농촌진흥청 ###
                        if "url" in next_btn_href:
                            print("## url - 마지막 페이지" )
                            next_exists = False

                
                except :
                    print("### 다음 버튼 없음 - 마지막 탭 ###")
                    next_exists = False
