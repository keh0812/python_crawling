...
class CrawlingPagingMain():
    ...
    def crawling_main(self, collection_data):
        ...
        # try:
            ...
            # 메뉴 이동2
            if 'menu2' in event_data:

                menu_list = event_data['menu2']
            ...
            # 메뉴 이동3
            if 'menu3' in event_data:

                menu_list = event_data['menu3']
            ...
            while next_exists:
                ...
                for items in paging_all_items :
                    
                    print("text : ", items.text)
                    # ">,>>" 특수문자 구분하여 저장하기
                    if items.text.isdecimal() == False :
                        
                        # 문자 길이 체크(2 이상 제외)
                        text_length = len(items.text)
                        if text_length < 2 :

                            paging_next_btn.append(items)
                            item_unicode = ord(items.text)
                            print(" item_unicode : ", ord(items.text), ",text : ", items.text)

                            # ">" ASCII 변환 값 : 62 또는 8250
                            if item_unicode == 62:
    
                                paging_next_btn[0] = items
                                print("paging_items 넥스트 값 : ", paging_next_btn[0])
                                print(paging_next_btn[0].text)
                                time.sleep(3)

                try :
                    
                    # ">" ASCII 변환 값 : 62 또는 8250
                    if ord(paging_next_btn[0].text) == 62:
                        paging_next_btn[0].click()
                        print("### 다음 버튼 클릭 ###")
                        time.sleep(2)
                    else : 
                        print("### 버튼 없음 - 마지막 페이지 ###")
