    ...
    def crawling_main(self, collection_data):
        ...
        # try:
            ...
            while next_exists:
                ...
                for paging_index in range (paging_items_cnt):
                    ...
                    if (detail_tag != ""):
                        ...
                        for i in range (item_cnt):
                            ...
                            biz_m_cd = ""
                            biz_s_cd = ""
                            if "biz_l_cd" in common_data:
                                biz_l_cd = common_data['biz_l_cd']
                            if "biz_m_cd" in common_data:
                                biz_m_cd = common_data['biz_m_cd']
                            if "biz_s_cd" in common_data:
                                biz_s_cd = common_data['biz_s_cd']

                            # 도큐먼트 ID
                            doc_id = s_cd + "_" + menu_cd + "_" + data_id
                            result_data['id'] = "nams_" + doc_id
                            # 보고서 분류체계 대분류 코드
                            result_data['biz_l_cd'] = biz_l_cd
                            # 보고서 분류체계 중분류 코드
                            result_data['biz_m_cd'] = biz_m_cd
                            # 보고서 분류체계 소분류 코드
                            result_data['biz_s_cd'] = biz_s_cd
                            # 데이터출처 대분류 코드
                ...
                try:    
                    next_js = driver.find_element_by_css_selector(next_tag).get_attribute('onclick')
                    next_js_text = re.sub(r'[^0-9]', '', next_js)
                    
                    paging_items_cnt = paging_items_cnt - 1
                    
                    ## 한 탭 내 마지막 페이지 확인 (탭 이동)
                    # 마지막 페이지가 아닐 때
                    if paging_index != paging_items_cnt :

                        print("변동없이 계속")
                    
                    # 탭 내 마지막 페이지 일 때
                    else:
                        if str(paging_num) == next_js_text :
                            print("### 마지막 페이지 ###")
                            next_exists = False
                        else :
