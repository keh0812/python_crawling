    ...
    def crawling_main(self, collection_data):
        ...
        # try:
            ...
            while next_exists:
                ...
                # 페이징 요소 확인
                # print(f'{paging_cnt} 카운트 시작')
                active_btn = driver.find_element_by_css_selector(active_page_tag)
                active_page = active_btn.text
                active_page = int(active_page)
                # print("active 페이지 : ", active_page)

               
                # 스크롤 다운
                # driver.execute_script("arguments[0].scrollIntoView(true);", paging_bar)
                if headless_height != "0":
                        driver.execute_script("window.scrollTo(0, window.scrollY - " + headless_height + ");")
                        time.sleep(2)

                # 페이지 확인
                paging_num =  active_page
