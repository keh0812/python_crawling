...
class CrawlingPagingMain():
    ...
    def crawling_main(self, collection_data):
        ...
        # try:
            ...
            # 총 페이지 수 추출
            total = "footer > nav > ul > li:nth-child(6) > a"
            page_total = driver.find_element_by_css_selector(total).text
            cnt = page_total[page_total.find("\n") + 1:]
            paging_cnt = int(cnt)
            print(paging_cnt, "===> 총 페이지 수")
            ...
            for paging_index in range (paging_cnt):
                ...
                # 페이징 요소 확인
                i = paging_index + 1
                url = 'https://www.whitehouse.gov/?s=north+korea&paged='
                next_url = url + str(i)

                driver.get(next_url)
                time.sleep(3)

                logger.info(f'crawling | crawling_main() | crawling | {l_cd}/{m_cd}/{s_cd}/{menu_cd} | {i} 페이지')
                create_log(crawling_type, l_cd + "/" + m_cd + "/" + s_cd + "/" + menu_cd, str(i) + " 페이지")
